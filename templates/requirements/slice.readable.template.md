# Срез — <Русское название смыслового среза>

Статус: **черновик**
Функциональность: `features/<feature-slug>/feature.md`
Порядок в требованиях функциональности: `<01>`
Дата обновления: `<YYYY-MM-DD>`
Формат: **новый лёгкий**
Шаблон: `templates/requirements/slice.readable.template.md`

## Назначение

<1-2 предложения: какую часть функциональности закрывает срез и зачем он выделен отдельно.>

## Главное

- Главный источник бизнес-правил: `../../requirements.md`.
- Срез содержит только границы, ссылки, проверяемые правила и основное направление тестирования.
- Если во время подготовки пакета найдено новое правило или противоречие, сначала обновить `../../requirements.md`, затем заново построить этот срез в том же проходе.

## Границы среза

| Входит | Не входит |
|---|---|
|  |  |

## Схема среза

```plantuml
@startuml
left to right direction
rectangle "<экран/сервис>" as A
rectangle "<действие>" as B
rectangle "<результат>" as C
A --> B
B --> C
@enduml
```

## Связанные плановые истории

- `STORY-<FEATURE>-NNN`

## Пакеты требований

- `../../requirements.md`
- `requirements/frontend.md`
- `requirements/backend.md`

## Связанные прототипы

- `../../planning/scope-prototype/prototype.html`
- `delivery-prototype/prototype.html`
- `delivery-prototype/notes.md`

## Фокус тестирования среза

- [ ] Проверить основной успешный сценарий.
- [ ] Проверить пустые состояния.
- [ ] Проверить ошибки API и недоступные действия.
- [ ] Проверить права ролей.
- [ ] Проверить отсутствие старых терминов/маршрутов/статусов, если срез заменяет прежнюю логику.

## Связанные артефакты исполнения

- `execution/tasks.md`
