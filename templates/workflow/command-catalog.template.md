...
This file documents short natural-language commands that switch context or trigger recurring workflow actions. All communication with the analyst remains in Russian unless the analyst explicitly requests another language.
...
| `начинаю работу над фичей <feature>` | `requirements` | Выполнить обязательную одноразовую миграцию, если она ещё не настроена, затем создать личную рабочую ветку функциональности от актуальной `origin/main`. |
| `сохрани работу` | текущий режим | Проверить и зафиксировать все точные изменения в активной рабочей ветке, затем отправить только эту ветку. |
| `обнови мою ветку` | текущий режим | С защитным снимком объединить актуальную `origin/main` с активной рабочей веткой и отправить её. |
| `требования готовы к объединению` | `requirements` | Проверить требования и отправить рабочую ветку; явно сообщить, что запрос на слияние не создан, а ссылка ведёт на форму его создания. |
| `запрос на слияние принят` | текущий режим | Доказать включение отправленного коммита в `origin/main`, завершить рабочую сессию и вернуться на актуальную `main`. |
...
| `начинаю работу над фичей <feature>` | `пишем требования по фиче <feature>`, `работаем с фичой <feature>`, `беру фичу <feature> в работу`, `берём фичу <feature> в работу`, `начинаем работу над требованиями по <feature>`, `начинаем работы над требованиями по <feature>` |
| `сохрани работу` | `сохрани`, `зафиксируй`, `зафиксируй изменения`, `закоммить`, `закоммить изменения`, `сделай коммит` |
| `обнови мою ветку` | `обнови`, `синкани`, `подтяни изменения`, `синкани мою ветку`, `обнови рабочую ветку` |
| `требования готовы к объединению` | `отправь требования на проверку`, `подготовь требования к слиянию`, `отправь ветку на слияние`, `вливаем в основную ветку`, `влей в основную ветку` |
| `запрос на слияние принят` | `MR принят`, `PR принят`, `ветка принята в main`, `ветка влита в main`, `слияние выполнено` |
...
|---|---|---|
| `начинаю работу над фичей <feature>` и синонимы | `requirements` | Выполнить `workspace.py bootstrap` и `collaboration.py status`. Если получено `migration-required`, запросить идентификатор аналитика, выполнить одноразовую миграцию и только затем `collaboration.py start`. До создания рабочей ветки не читать и не изменять требования, срезы и пакеты. |
| `синкани репы` | текущий аналитический режим | Выполнить `workspace.py bootstrap`, затем `workspace.py sync`. Если выбрана отправленная рабочая ветка, sync сначала вызывает `finish`: доказанное включение коммита в `origin/main` закрывает рабочую сессию и позволяет продолжить обмен из `main`, недоказанное включение блокирует обновление `code` и `source`. Неотправленная рабочая ветка также блокирует обмен. Если одноразовая миграция ещё не настроена, грязную или расходящуюся `main` сначала сохранить через миграцию в ветку функциональности, не коммитить её напрямую. При конфликте analytics-origin использовать только `inspect-analytics-origin-conflict`, решения запрашивать по одному. Отсутствующие `code` и `source` не восстанавливать. Корневую обвязку не изменять и не коммитить. |
...
| `сформируй пакет для разработки` | `requirements` | В многопользовательском режиме сначала доказать принятие рабочей ветки через `collaboration.py finish` и актуальность `main` через `require-main-for-delivery`; при не принятой ветке ничего не производить. Затем проверить полноту, непротиворечивость, проверяемость, влияния, трассировку и язык корневого документа; автоматически исправить только однозначное; при сомнениях задавать по одному вопросу; построить срезы и опубликовать редакцию как `sent`. |
