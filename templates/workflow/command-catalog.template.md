...
| `начинаю работу над фичей <feature>` | `requirements` | В многопользовательском режиме создать личную рабочую ветку функциональности от актуальной `origin/main`. |
...
| `начинаю работу над фичей <feature>` | `пишем требования по фиче <feature>`, `работаем с фичой <feature>`, `беру фичу <feature> в работу`, `берём фичу <feature> в работу` |
...
|---|---|---|
| `синкани репы` | текущий аналитический режим | Выполнить `workspace.py bootstrap`, затем `workspace.py sync`. Полный обмен разрешён только из `analytics/main`; активная рабочая ветка блокирует команду до обновления даже `code`. В однопользовательском режиме понятные изменения `analytics` предварительно проверять и фиксировать только точными путями. В многопользовательском режиме грязную или расходящуюся `main` сначала сохранять через миграцию в ветку функциональности, не коммитить её напрямую. При конфликте analytics-origin использовать только `inspect-analytics-origin-conflict`, решения запрашивать по одному. Отсутствующие `code` и `source` не восстанавливать. Корневую обвязку не изменять и не коммитить. |
