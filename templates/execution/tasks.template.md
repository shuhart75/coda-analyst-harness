...
- Если аналитик подтвердил завершение к определённой дате, но точный фактический интервал неизвестен, добавь необязательную колонку `Completed By` с границей `YYYY-MM-DD`, `Status = done` и `Progress % = 100`. Запиши источник подтверждения в Notes/Details. Это верхняя граница, не `Actual Finish`: неизвестные фактические даты остаются пустыми. Без полного фактического интервала генератор показывает подписанную отметку границы без резервирования ресурса; плановый интервал не подставляется. Известный `Actual Start` по-прежнему привязывает начало PLAN, сама граница его не привязывает.
- `cancelled`/`canceled` и `superseded` не планируются и не входят в расчёт процента PLAN. Исходные записи и связи сохраняются; генератор оставляет комментарий, а не активную полосу. Отмена не означает завершение на 100%.
