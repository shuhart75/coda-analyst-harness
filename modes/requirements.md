...
Дополнительные равнозначные формулировки: `отправь требования в разработку`, `передай требования разработчикам`, `передай разрабам`, `отдай требования разрабам`, `отправь разрабам`, `отдаём в разработку`. До пункта 1 выполни `collaboration.py finish`, если рабочая ветка уже принята, и затем `collaboration.py require-main-for-delivery --feature <feature>`. Отсутствие настройки совместной работы является блокирующей незавершённой миграцией. Если ветка ещё не входит в `origin/main`, пакет, срезы и подробные приложения не создавай: сначала требуется запрос на слияние и его принятие человеком.
