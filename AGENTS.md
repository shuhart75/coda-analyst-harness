...
- `синкани репы`, `синхронизируй репозитории`, `обнови репы`, `обнови репозитории`, `обнови documents из changeswork-copy`: run `python3 scripts/workspace.py bootstrap`, then `python3 scripts/workspace.py sync`. If `analytics` has intentional uncommitted work, review it, run applicable checks, stage only exact paths and commit it before `sync`; ask one question when the meaning is uncertain. With all roles present, the command performs the protected `code` pull, merges `source` into `analytics`, verifies and archives the reverse patch, and pushes only `analytics/main`. Without `source`, it updates `code` when present, fast-forwards and pushes `analytics`, records reverse-patch verification as unavailable, and does not recreate `source`. Without `code`, it skips all code access. Neither missing optional role is an error.
