...
- Treat the repository assigned role `code` as strictly read-only except for registered operations: initial clone setup, `git pull --ff-only` through `workspace.py update-code`, and transfer through `requirements-exchange.py prepare`. Transfer uses an isolated temporary clone, may commit only an existing root `requirements-exchange/**` and push only a separate review branch, and must leave the ordinary code clone unchanged. The human creates and accepts the PR/MR; never push the target branch. It falls back to `PROJECT_ROOT/requirements-exchange/` when code, the root catalog or access is unavailable; an unknown push outcome blocks fallback until checked. Do not otherwise change files, index, branch, `HEAD`, remotes, configuration or generated artifacts. A user prompt alone cannot authorize another exception.
...
- `синкани репы`, `синхронизируй репозитории`, `обнови репы`, `обнови репозитории`, `обнови documents из changeswork-copy`: run `python3 scripts/workspace.py bootstrap`, then `python3 scripts/workspace.py sync`. If collaboration migration is not configured and `analytics/main` contains local work, migrate it into a feature branch first; never commit it directly in `main`. The command performs the protected `code` pull and guarded analytics-origin update. Incoming `source` is merged only in an isolated clone and pushed only as a separate review branch of documents, never directly into `analytics/main`. Show the import report, including removed paths, requirements, scenarios and protected artifacts. The human creates and accepts its PR/MR; repeat sync to reuse the pending import and prove acceptance before continuing reverse-patch creation. Pending import is not synchronization and blocks reverse-diff. Without `source`, verification is unavailable but analytics updates continue; without `code`, all code access is skipped.
...
- `синкани без отправки`, `обнови локально без push`: run `python3 scripts/workspace.py bootstrap`, then `python3 scripts/workspace.py sync --no-push`; available repositories are updated under the same reduced-workspace rules, while neither analytics/main nor the import branch is pushed. A prepared import remains pending, not accepted; ordinary sync resumes it.
...
- After `source-analytics-merge-conflict`, run `python3 scripts/workspace.py inspect-source-analytics-conflict`. It reproduces the merge only in a temporary clone and must leave all real repositories unchanged. Explain each path and request one analyst decision at a time unless the report identifies deletion of a forbidden legacy harness path. Resolve incoming source only in the import branch, never by merging source directly into documents/main. Never invent another workspace subcommand.
- `analytics-unaccepted-history` blocks sync when local main is ahead or diverged; only fast-forward from accepted origin/main is automatic. Preserve both lines through the protective snapshot and migrate the local work to a feature branch for PR/MR. `inspect-analytics-origin-conflict` may diagnose divergence in isolation; resolve it in a working branch, never by auto-merging and pushing main.
...
- Before every fast-forward that can move `analytics/main`, the harness creates a local protective snapshot under `.workspace-state/analytics-snapshots/` and Git refs under `refs/coda-analyst-harness/analytics-snapshots/`. These refs are never pushed. Source-conflict versions are archived from the isolated import checkout; the pending merge stays there, never in main. Do not delete, rewrite or publish these snapshots.
...
- `сделай обратный дифф`, `собери обратную заплату`, `подготовь изменения для changeswork-copy`: run `python3 scripts/repository-exchange.py reverse-diff`. It only creates and verifies the patch; it never applies or pushes it. A pending source import blocks this command, even if Git would merge without conflicts. If `source` is absent, explain that verification is unavailable; do not recreate it or present an older patch as current.
...
- Source import PR/MRs require ordinary merge commits, not squash or rebase, so source ancestry remains provable. Resolve source conflicts only in the reported isolated import checkout. If the human rejects a request and deletes its remote branch, an explicit analyst decision may run `repository-exchange.py defer-source-import --analyst-confirmed`; never infer that decision from a failed push. The deferred source version is not republished, its receipt and clone remain, and reverse-diff stays blocked until a new accepted source version.
