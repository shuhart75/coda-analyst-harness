...
- `синкани репы`, `синхронизируй репозитории`, `обнови репы`, `обнови репозитории`, `обнови documents из changeswork-copy`: run `python3 scripts/workspace.py bootstrap`, then `python3 scripts/workspace.py sync`. If `analytics` has intentional uncommitted work, review it, run applicable checks, stage only exact paths and commit it before `sync`; ask one question when the meaning is uncertain. With all roles present, the command performs the protected `code` pull, merges `source` into `analytics`, verifies the reverse patch and pushes only `analytics/main`. Without `source`, it updates `code` when present, fast-forwards and pushes `analytics`, records the reverse patch as unavailable, and does not recreate `source`. Without `code`, it skips all code access. Neither missing optional role is an error.
...
- `сделай обратный дифф`, `собери обратную заплату`, `подготовь изменения для changeswork-copy`: run `python3 scripts/repository-exchange.py reverse-diff`. Do not apply or push the patch unless the user separately asks. If `source` is absent, explain that verification is unavailable; do not recreate it or present an older patch as current.
