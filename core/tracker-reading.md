...
провайдеру. Для Jira сборщик делает один batch-вызов на полный набор ключей job;
для SberTrek — ровно один history-вызов на ключ. Канонический evidence, точный
набор ключей и его SHA-256 записываются в `calls` job и сверяются с MCP-журналом.
Один Jira batch-ответ нельзя переименовать в несколько per-key evidence.
