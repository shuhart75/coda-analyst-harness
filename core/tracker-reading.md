...
SHA-256. Сборщик следует только `core/tracker-collector.md` и job-файлу. Он не
читает общие правила проекта или документацию MCP.
...
Поисковый ответ нормализуется без detail-вызовов каждой задачи. Provider-файл
хранит только ключи, название, тип, статус, исполнителя, оценку, эпик, релизы,
даты создания/обновления, состояния `value/absent/not-returned` и evidence.
При этом описания, комментарии, вложения и полный MCP-ответ не сохраняются.
