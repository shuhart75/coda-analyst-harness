...
задачам роли в фиче, без `superseded`, `cancelled`/`canceled` и кандидатов. Дробные оценки не повышаются до 1.
Подтверждённый факт завершённой BE/FE-задачи может отображаться без оценки;
неизвестная оценка не заменяется единицей или фактической длительностью.
Взвешенный прогресс тогда неизвестен, кроме случая 100% у всех учитываемых задач.

Необязательное поле задачи `Completed By` хранит подтверждённую аналитиком верхнюю
границу завершения (`YYYY-MM-DD`), не точную дату `Actual Finish`. Оно требует
завершённого статуса и 100% прогресса. Без полного фактического интервала задача
...
существующей работой, а не автоматически порождает новую. Правила фактических
дат по истории и сохранения закрытой разработки при возвратах QA описаны
в `core/tracker-lifecycle.md`; адаптер истории пока не подключён.

Для явной миграции смешанных story-полос на существующие ролевые полосы плана
используй `core/role-plan-baselines.md`: отдельные привязки схемы 4 сохраняют
старые карты и заменяют только сравнительный слой диаграммы. Квартальный источник
без командирского зазора требует подтверждения; исторические и новые объёмы одной
фичи не объединяются автоматически.
